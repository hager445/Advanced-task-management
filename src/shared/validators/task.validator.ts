...
return from(supabase.from('tasks').select('*').eq('assigned_to ',authService.$user_id())).pipe(map(({data,error})=>{
    const exists = data?.some(task => {
    if (editaddformsService.isEdit.taskForm() && task.task_id === taskService.$updatedTask().task_id){
   return false ;
    }
